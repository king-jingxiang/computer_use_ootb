...
CONFIG_DIR = Path("~/.computer_use_ootb").expanduser()
...
def setup_state(state):
    ...
    if "model" not in state:
        # TODO 替换成自定义模型
        state["model"] = "qwen2-vl-max + ShowUI"
        # _reset_model(state)
...
def process_input(user_input, state):
    ...
    for loop_msg in sampling_loop_sync(
            ...
            output_callback=partial(chatbot_output_callback, chatbot_state=state['chatbot_messages'],
    ...
    with gr.Accordion("Quick Start Prompt", open=False):  # open=False 表示默认收
        ...
        with gr.Row():
            ...
            with gr.Column(scale=1):
                ...
                image_preview = gr.Image(value=initial_image_value, label="Reference Initial State",
